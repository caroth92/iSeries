...
class MyEpisodeViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [AnyObject]? {
        var watchedAction = UITableViewRowAction(style: UITableViewRowActionStyle.Default, title: "Watched") { (action: UITableViewRowAction!, indexPath: NSIndexPath!) -> Void in

            let episodeID = self.episodes[indexPath.row].objectId as NSString
            
            var query = PFQuery(className: "UserHistorial")
            query.whereKey("EpisodeId", equalTo: episodeID)
            query.findObjectsInBackgroundWithBlock{(objects:[AnyObject]!,error:NSError!) -> Void in
                if error == nil {
                    var episodeObject = objects.first as PFObject
                    episodeObject["seen"] = true
                    episodeObject.save()
                }
            }
    ...
    func loadEpisode(callback: (PFObject, NSError!) -> ()) {
        var query = PFQuery(className: "UserHistorial")
        query.whereKey("EpisodeId", equalTo: self.serieID)
